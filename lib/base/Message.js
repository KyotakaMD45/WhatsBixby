...
class Message extends Base {
  ...
  async SetFullPP(jid, imageBuffer) {
  try {
    const { img } = await this.generateProfilePicture(imageBuffer);
    
    // Update profile picture using Baileys' built-in method
    await this.client.updateProfilePicture(jid, img);
...
async generateProfilePicture(imageBuffer) {
  try {
    const image = await Jimp.read(imageBuffer);
    const img = await image
      .clone()
      .cover(640, 640) // WhatsApp full profile picture dimensions
